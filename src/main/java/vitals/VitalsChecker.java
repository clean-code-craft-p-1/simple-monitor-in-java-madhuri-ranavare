...
public class VitalsChecker {
    ...
    static String outOfRangeMessage(float temperature, float pulseRate, float spo2) {
        if (!isInRange(temperature, 95, 102)) return "Temperature is critical!";
        if (!isInRange(pulseRate, 60, 100)) return "Pulse Rate is out of range!";
        if (!isInRange(spo2, 90, 100)) return "Oxygen Saturation out of range!";
        return null;
    }
